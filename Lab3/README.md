...
* [Old Autograder Marking Process](#old-autograder-marking-process)
  * [Autograder Marks](#autograder-marks)
* [Extra Ciphertexts](#extra-ciphertexts)

## Background

In this lab, you will get experience creating a design that contains several on-chip memories and uses a more efficient ready-enable interface to connect modules with variable latencies.
...
In the rest of the lab, you will first design an ARC4 decryption circuit. In
the first phase, the secret key will be obtained from a bank of switches on
your DE1-SoC board, and the encrypted message will be given to you as a memory
initialization file.  Next, you will extend this to build an ARC4 _cracking_
circuit; the circuit will implement a “brute-force” attack by cycling through
the entire key space and stopping when a successful decryption is performed.
Finally, you will create a cracking circuit with multiple decryption units to
speed things up.
...
marking process below for you, so you know what you're missing. However, you **must** keep the
file names and structure as per the instructions in the [Old Autograder Marking Process](#old-autograder-marking-process)
given below.

Your grade will have two components:

1. Demo and interview during your lab session: 10 marks (breakdown by task below)
2. Code quality check: grade multiplier in the range of 0 to 1
2. Individual multiplier based on answers to TA's questions: grade multiplier in the range of 0 to 1

The TAs may ask you to decipher any of the messages in [Extra ciphertexts](#extra-ciphertexts), or other messages encoded similarly.
...
## Old Autograder Marking Process

The autograder will not be used. This section is kept intact for
legacy purposes.
...
The autograder will instantiate and test each module exactly the way it is
defined in the provided skeleton files. This means that
...
**For Task 5 only**, you may modify the `crack` port list **only by adding
additional ports**; you may not remove or rename any ports already defined
there, or rename any pre-defined instances.
...
### Autograder Marks (Not used in 2025w, see Canvas notes on grading lab 3)
